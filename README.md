...
The public key can be stored in the repository variable. The release workflow accepts either the raw
minisign public key block or the base64-wrapped value generated by `--ci`, and normalizes it before
building updater artifacts. The private key content or path is only for local builds; in CI, store
the private key content in the GitHub secret.
